...
public class TestCore extends Page{
		...
		if(config.getProperty("Platform").toLowerCase().equals("android")){
			...
			if(config.getProperty("Application_Type").toLowerCase().equals("wap")){
				...
				if(config.getProperty("Browser").toLowerCase().equals("chrome")){
					...
					log("----------- Launching Firefox Browser -------------", ILogLevel.TESTCASE);
				}

			}
			else if(config.getProperty("Application_Type").toLowerCase().equals("app")){
				File classpathRoot = new File(System.getProperty("user.dir"));
				File app = new File(classpathRoot, "/app/Shuttl.apk");

				DesiredCapabilities capabilities = new DesiredCapabilities();
				capabilities.setCapability("deviceName", "52034220fefd7327");
				capabilities.setCapability("platformVersion", "8.1.0");
				capabilities.setCapability("appPackage", "app.goplus.in.myapplication");
				...
				log("----------- Launching Shuttl app on device-------------", ILogLevel.TESTCASE);
	...
	public void setUp(Method method) throws IOException, InterruptedException, MalformedURLException {
		...
		if(config.getProperty("Platform").toLowerCase().equals("windows")||config.getProperty("Platform").toLowerCase().equals("linux")||config.getProperty("Platform").toLowerCase().equals("wap")||config.getProperty("Platform").toLowerCase().equals("mac")){
